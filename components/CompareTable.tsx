...
export default function CompareTable({ funds, accentColor, compact, selectedYears }: CompareTableProps) {
  ...
  return (
    <div style={{ overflowX: "auto", marginBottom: compact ? 10 : 24, ...(compact ? { pageBreakInside: "avoid", breakInside: "avoid" } as React.CSSProperties : {}) }}>
        ...
        <thead>
          <tr>
            <th style={{
              ...
              minWidth: compact ? 70 : 110, backgroundColor: "white",
            }}>
              מדד
            </th>
            {funds.map((f, i) => (
              <th key={f.id} style={{
                ...
                minWidth: compact ? 80 : 130,
                borderRadius: !compact ? (i === 0 ? "8px 0 0 0" : i === funds.length - 1 ? "0 8px 0 0" : 0) : 0,
              }}>
                {f.name}
              </th>
            ))}
      ...
      {!compact && (
        <div style={{ marginTop: 8, fontSize: fs.legend, color: "#8893a4", display: "flex", alignItems: "center", gap: 4 }}>
          <span style={{ color: accentColor }}>★</span> ערך מוביל בקטגוריה
        </div>
      )}
    </div>
  );
