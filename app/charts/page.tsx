...
function InsightsBlock({
  ...
  interface Insight { text: string; icon: string; isFeatured: boolean; }
  ...
  if (withSharpe.length > 0) {
    ...
    insights.push({
      text: `הקרן הבולטת: ${best.name} — שארפ ${best.sharpe!.toFixed(2)}, תשואה ${best.x.toFixed(2)}%, סטיית תקן ${best.y.toFixed(2)}%`,
      icon: "✦",
      isFeatured: true,
    });
  }
  ...
  if (currencyFilter === "all") {
    ...
    if (currencies.size >= 2) {
      insights.push({
        text: "הגרף כולל קרנות בשקל ובדולר — ההשוואה עלולה להיות מטעה",
        icon: "⚠️",
        isFeatured: false,
      });
    }
    ...
    <div className="no-print" style={{
      ...
      {insights.map((insight, i) => (
        <div key={i} style={{
          ...
          gap: 10,
          padding: insight.isFeatured ? "10px 12px" : "6px 4px",
          marginBottom: insight.isFeatured ? 4 : 2,
          background: insight.isFeatured
            ? (dark ? "rgba(74,222,128,0.08)" : "rgba(27,58,47,0.04)")
            : "transparent",
          borderRadius: insight.isFeatured ? 10 : 0,
          borderRight: insight.isFeatured ? `3px solid ${dark ? "#4ade80" : "#1B3A2F"}` : "none",
          fontSize: 13,
          color: dark ? "#c9d8cf" : "#1a2e26",
          lineHeight: 1.6,
        }}>
          <span style={{
            fontSize: insight.icon === "⚠️" ? 13 : 14,
            flexShrink: 0,
            marginTop: 2,
            color: insight.icon === "⚠️"
              ? "#f59e0b"
              : (insight.isFeatured
                ? (dark ? "#4ade80" : "#1B3A2F")
                : (dark ? "rgba(74,222,128,0.5)" : "#9ca3af")),
          }}>
            {insight.icon}
          </span>
...
function ChartsContent() {
  ...
  /* shared small select style for period controls */
  ...
  return (
    ...
    <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              />
              <span style={{ position: "absolute", right: 9, top: "50%", transform: "translateY(-50%)", fontSize: 11, color: "#9ca3af", pointerEvents: "none" }}>
                🔍
              </span>
              {search && (
                <button
                  onClick={() => setFilter("search", "")}
                  style={{ position: "absolute", left: 8, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "#9ca3af", fontSize: 12, padding: 0, lineHeight: 1 }}
                >✕</button>
              )}
            </div>
            ...
            <div style={{ position: "relative" }}>
              <select
                ...
                style={{
                  ...selectStyle,
                  paddingRight: 22,
                  border: group !== ALL ? `1.5px solid ${primary}` : "1px solid #e2e8f0",
                  color: group !== ALL ? primary : "#6b7280",
                  fontWeight: group !== ALL ? 600 : 400,
                  background: group !== ALL ? `${primary}08` : "#fff",
            ...
            <div style={{ position: "relative" }}>
              <select
                ...
                style={{
                  ...selectStyle,
                  paddingRight: 22,
                  border: category !== ALL ? `1.5px solid ${primary}` : "1px solid #e2e8f0",
                  color: category !== ALL ? primary : "#6b7280",
                  fontWeight: category !== ALL ? 600 : 400,
                  background: category !== ALL ? `${primary}08` : "#fff",
                  appearance: "none", WebkitAppearance: "none",
                }}
              >
            ...
            {activeFilterCount > 0 && (
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${primary}30`, background: `${primary}08`,
                  color: primary, fontSize: 11, fontWeight: 600, cursor: "pointer",
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: 5, alignItems: "center", overflow: "hidden", flexShrink: 1, minWidth: 0 }}>
            {options.classifications.length >= 2 && [ALL, ...options.classifications].map((cls) => (
              <button key={cls}
                onClick={() => setFilter("classification", cls)}
                style={pillStyle(classification === cls)}
              >{cls === ALL ? "הכל" : cls}</button>
            ))}
          </div>
            ...
            <div style={{ display: "flex", gap: 3 }}>
              {(["all", "ILS", "USD"] as const).map((c) => (
                <button key={c}
                  ...
                  style={pillStyle(currencyFilter === c)}
                >{c === "all" ? "הכל" : c}</button>
              ))}
            </div>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px" }}>
        ...
        {/* Meta text — below controls, above chart */}
        <div className="no-print" style={{
          direction: "rtl", padding: "10px 0 0",
          fontSize: 12, color: "#9ca3af",
          display: "flex", alignItems: "center", gap: 6,
        }}>
          <span style={{ fontVariantNumeric: "tabular-nums" }}>{points.length} קרנות</span>
...
function PrintLegend({ points, periodLabel, dark }: { points: ScatterPoint[]; periodLabel: string; dark: boolean }) {
  ...
  return (
    <table className="print-only-table" style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, direction: "rtl", marginBottom: 20 }}>
      ...
      <tbody>
        {sorted.map((p, i) => {
          const borderRight = p.rank === "top" ? `3px solid ${dark ? "#4ade80" : "#1B3A2F"}` : p.rank === "bottom" ? `3px solid ${dark ? "#f87171" : "#DC2626"}` : "3px solid transparent";
          return (
            <tr
              key={p.name}
              style={{
            ...
            >
              <td style={{ padding: "14px 16px", textAlign: "center", fontWeight: 600, fontSize: 11, color: "var(--text-muted)", fontVariantNumeric: "tabular-nums", opacity: 0.5 }}>
                {i + 1}
              </td>
              <td style={{ padding: "14px 16px", fontWeight: 500, color: "var(--text-primary)" }}>
                {p.name}
              </td>
              <td style={{ padding: "14px 16px", textAlign: "center", fontWeight: 700, fontSize: 14, color: "var(--text-primary)", fontVariantNumeric: "tabular-nums" }}>
                {p.x.toFixed(2)}%
              </td>
