...
function AdminContent() {
  ...
  const [activeTab, setActiveTab] = useState<"data" | "funds" | "branding" | "settings" | "monthly-history" | "ai-parser" | "benchmarks" | "data-completion">("data");
  ...
  return (
    <div style={{ minHeight: "100vh" }}>
        ...
        <div style={{ display: "flex", gap: 4, borderBottom: "1px solid var(--border)" }}>
          {[
            ...
            ...(role === "super" ? [
              ...
              { id: "data-completion" as const, label: "השלמת נתונים" },
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "20px 24px" }}>
        ...
        {activeTab === "data-completion" && (
          <DataCompletionTab data={data} onUpdate={(updatedData) => { setData(updatedData); setDirty(true); setSaved(false); }} onStatus={showStatus} />
        )}
...
/*  Data Completion Tab                                                */
/* ================================================================== */
function DataCompletionTab({ data, onUpdate, onStatus }: {
  data: FundsData;
  onUpdate: (data: FundsData) => void;
  onStatus: (msg: string) => void;
}) {
  const [computing, setComputing] = useState(false);

  // Scan all active funds for missing computed fields
  const analysis: {
    catId: string; catName: string; fund: Fund;
    missingAvg: boolean; missingStd: boolean; missingSharpe: boolean;
    monthCount: number; yearCount: number;
    canComputeAvg: boolean; canComputeStd: boolean; canComputeSharpe: boolean;
    computedAvg: number | null; computedStd: number | null; computedSharpe: number | null;
  }[] = [];
  ...
  for (const cat of data.categories) {
    for (const fund of cat.funds) {
      ...
      if (canComputeStd) {
        ...
        computedStd = Math.sqrt(variance) * Math.sqrt(12); // annualized
      }
  ...
  const totalComputable = analysis.filter(a => a.canComputeAvg || a.canComputeStd || a.canComputeSharpe).length;

  const handleCompute = () => {
    if (totalComputable === 0) return;
    if (!window.confirm(`להשלים נתונים מחושבים ל-${totalComputable} קרנות?\n\nממוצע שנתי — מממוצע התשואות השנתיות\nסטיית תקן — מנתונים חודשיים (שנתי)\nשארפ — ממוצע שנתי / סטיית תקן\n\nהנתונים ייכנסו כטיוטה — יש ללחוץ "שמירה ופרסום" לאישור.`)) return;
    ...
    const newData: FundsData = {
      ...
      categories: data.categories.map((cat) => ({
        ...cat,
        funds: cat.funds.map((fund) => {
          const entry = analysis.find(a => a.fund.id === fund.id && a.catId === cat.id);
    ...
    onUpdate(newData);
    onStatus(`✓ הושלמו נתונים ל-${updated} קרנות — שמור לפרסום`);
    setComputing(false);
  };

  // Count all funds missing data (even if can't compute)
  const allMissing: { fund: Fund; catName: string; fields: string[] }[] = [];
  for (const cat of data.categories) {
    for (const fund of cat.funds) {
      if (fund.active === false) continue;
      const missing: string[] = [];
      if (fund.avgAnnualReturn === null || fund.avgAnnualReturn === undefined) missing.push("ממוצע שנתי");
      if (fund.stdDev === null || fund.stdDev === undefined) missing.push("סטיית תקן");
      if (fund.sharpe === null || fund.sharpe === undefined) missing.push("שארפ");
      if (missing.length > 0) allMissing.push({ fund, catName: cat.name, fields: missing });
    }
  }
  ...
  return (
    <>
      ...
      <div style={{
        ...
        borderRadius: 10, padding: "14px 20px", display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap",
      }}>
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 11, color: "var(--text-muted)", margin: 0 }}>
            {allMissing.length === 0
              ...
              : `${allMissing.length} קרנות עם נתונים חסרים, ${totalComputable} ניתנות להשלמה אוטומטית`
            }
          </p>
        </div>
        {totalComputable > 0 && (
          <button
            ...
            disabled={computing}
            style={{
              backgroundColor: "#059669", color: "#fff", fontWeight: 600, padding: "8px 20px",
              borderRadius: 6, border: "none", cursor: computing ? "default" : "pointer",
              fontSize: 12, opacity: computing ? 0.5 : 1,
            }}>
            {computing ? "מחשב..." : `השלם ${totalComputable} קרנות`}
          </button>
        )}
      </div>
      ...
      {analysis.length > 0 && (
        <div style={{
          ...
          <div style={{ backgroundColor: "var(--bg-section)", color: "#fff", padding: "8px 16px", fontWeight: 600, fontSize: 12 }}>
            קרנות להשלמה ({analysis.length})
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            ...
            <tbody>
              {analysis.map((a) => (
                <tr key={`${a.catId}-${a.fund.id}`} style={{ borderBottom: "1px solid var(--border-table)" }}>
      ...
      {/* Funds with missing data that can't be computed */}
      {allMissing.filter(m => !analysis.some(a => a.fund.id === m.fund.id)).length > 0 && (
        <div style={{
          backgroundColor: "var(--bg-surface)", border: "1px solid var(--border)",
          borderRadius: 10, overflow: "hidden",
        }}>
          <div style={{ backgroundColor: "#f59e0b30", padding: "8px 16px", fontWeight: 600, fontSize: 12, color: "#f59e0b" }}>
            קרנות עם נתונים חסרים — אין מספיק מידע לחישוב
          </div>
          <div style={{ padding: "8px 16px" }}>
            {allMissing.filter(m => !analysis.some(a => a.fund.id === m.fund.id)).map((m) => (
